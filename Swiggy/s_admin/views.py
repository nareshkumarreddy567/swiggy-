...
def pending_res(request):
    ...
    return render(request,'s_admin/pending_status.html',{'data':rs})
...
def show_approve_res(request):
    rs = RestaurantModel.objects.filter(restro_status='approved')
    return render(request, 's_admin/approved_status.html', {'data': rs})


def show_cancel_res(request):
    rs = RestaurantModel.objects.filter(restro_status='cancel')
    return render(request, 's_admin/cancel_status.html', {'data': rs})
